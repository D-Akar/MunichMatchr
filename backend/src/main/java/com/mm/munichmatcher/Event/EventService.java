...
public class EventService {
    ...
    public List<Event> getFilteredEvents(User user) {
        ...
        return getAllEvents().stream().filter(event -> event.getInterests().stream().anyMatch(interests::contains) && event.getLanguages().stream().anyMatch(userLanguagues::contains) && List.of(event.getTypeOfEvent()).stream().anyMatch(userPrefTypes::contains)).toList();
    }
